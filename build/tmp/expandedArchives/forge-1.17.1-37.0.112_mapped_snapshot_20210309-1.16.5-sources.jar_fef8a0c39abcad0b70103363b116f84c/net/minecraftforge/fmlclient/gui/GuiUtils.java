...
public class GuiUtils
{
    ...
    public static void drawTexturedModalRect(PoseStack matrixStack, int x, int y, int u, int v, int width, int height, float zLevel)
    {
        ...
        wr.m_85982_(matrix, x        , y + height, zLevel).m_7421_( u          * uScale, ((v + height) * vScale)).m_5752_();
        wr.m_85982_(matrix, x + width, y + height, zLevel).m_7421_((u + width) * uScale, ((v + height) * vScale)).m_5752_();
        wr.m_85982_(matrix, x + width, y         , zLevel).m_7421_((u + width) * uScale, ( v           * vScale)).m_5752_();
        wr.m_85982_(matrix, x        , y         , zLevel).m_7421_( u          * uScale, ( v           * vScale)).m_5752_();
        ...
        if (!textLines.isEmpty())
        {
            ...
            if (needsWrap)
            {
                ...
                for (int i = 0; i < textLines.size(); i++)
                {
                    ...
                    List<FormattedText> wrappedLine = font.m_92865_().m_92414_(textLine, tooltipTextWidth, Style.f_131099_);
                    if (i == 0)
            ...
            MultiBufferSource.BufferSource renderType = MultiBufferSource.m_109898_(Tesselator.m_85913_().m_85915_());
            pStack.m_85837_(0.0D, 0.0D, zLevel);
            ...
            for (int lineNumber = 0; lineNumber < textLines.size(); ++lineNumber)
            {
                ...
                if (line != null)
                    font.m_92733_(Language.m_128107_().m_5536_(line), (float)tooltipX, (float)tooltipY, -1, true, mat, renderType, false, 0, 15728880);
    ...
    public static void drawGradientRect(Matrix4f mat, int zLevel, int left, int top, int right, int bottom, int startColor, int endColor)
    {
        ...
        buffer.m_85982_(mat, right,    top, zLevel).m_85950_(startRed, startGreen, startBlue, startAlpha).m_5752_();
        buffer.m_85982_(mat,  left,    top, zLevel).m_85950_(startRed, startGreen, startBlue, startAlpha).m_5752_();
        buffer.m_85982_(mat,  left, bottom, zLevel).m_85950_(  endRed,   endGreen,   endBlue,   endAlpha).m_5752_();
        buffer.m_85982_(mat, right, bottom, zLevel).m_85950_(  endRed,   endGreen,   endBlue,   endAlpha).m_5752_();
        tessellator.m_85914_();
    ...
    public static void drawInscribedRect(PoseStack mStack, int x, int y, int boundsWidth, int boundsHeight, int rectWidth, int rectHeight, boolean centerX, boolean centerY)
    {
        ...
        GuiComponent.m_93160_(mStack, x, y, boundsWidth, boundsHeight, 0.0f,0.0f, rectWidth, rectHeight, rectWidth, rectHeight);
    }
}
