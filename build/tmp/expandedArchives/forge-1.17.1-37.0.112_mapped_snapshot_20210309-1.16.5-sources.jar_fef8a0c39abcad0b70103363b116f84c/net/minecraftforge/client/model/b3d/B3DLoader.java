...
public enum B3DLoader
{
    ...
    public UnbakedModel loadModel(ResourceLocation modelLocation) throws Exception
    {
        ...
        if(!cache.containsKey(file))
        {
            ...
            try
            {
                try
                {
                    resource = manager.m_142591_(file);
                }
                catch(FileNotFoundException e)
                {
                    if(modelLocation.m_135815_().startsWith("models/block/"))
                        resource = manager.m_142591_(new ResourceLocation(file.m_135827_(), "models/item/" + file.m_135815_().substring("models/block/".length())));
                    else if(modelLocation.m_135815_().startsWith("models/item/"))
                        resource = manager.m_142591_(new ResourceLocation(file.m_135827_(), "models/block/" + file.m_135815_().substring("models/item/".length())));
                    else throw e;
                }
    ...
    public static final class B3DState implements ModelState
    {
        ...
        public B3DState(@Nullable Animation animation, int frame, int nextFrame, float progress, @Nullable ModelState parent)
        {
            ...
            this.progress = Mth.m_14036_(progress, 0, 1);
        ...
        public Transformation getPartTransformation(Object part)
        {
            ...
            {
                return parent.getPartTransformation(part).m_121096_(nodeTransform);
            }
    ...
    static final class NodeJoint implements IJoint
    {
        ...
        {
            Matrix4f m = new Transformation(node.getPos(), node.getRot(), node.getScale(), null).m_121104_();
            m.m_27657_();
    ...
    private static final class ModelWrapper implements UnbakedModel
    {
        ...
        @Override
        public Collection<Material> m_5500_(Function<ResourceLocation, UnbakedModel> modelGetter, Set<com.mojang.datafixers.util.Pair<String, String>> missingTextureErrors)
        {
            return textures.values().stream().filter(loc -> !loc.startsWith("#"))
                    .map(t -> new Material(TextureAtlas.f_118259_, new ResourceLocation(t)))
        ...
        public BakedModel m_7611_(ModelBakery bakery, Function<Material, TextureAtlasSprite> spriteGetter, ModelState modelTransform, ResourceLocation modelLocation)
        {
            ...
            for(Map.Entry<String, String> e : textures.entrySet())
            {
                ...
                {
                    builder.put(e.getKey(), spriteGetter.apply(new Material(TextureAtlas.f_118259_, new ResourceLocation(e.getValue()))));
                }
            }
    ...
    private static final class BakedWrapper implements IDynamicBakedModel
    {
        ...
        private void generateQuads(ImmutableList.Builder<BakedQuad> builder, Node<?> node, final ModelState state, ImmutableList<String> path)
        {
            ...
            if(node.getKind() instanceof Mesh && meshes.contains(node.getName()) && state.getPartTransformation(Models.getHiddenModelPart(newPath)).isIdentity())
            {
                ...
                {
                    private final Transformation global = state.m_6189_();
                    ...
                    {
                        return global.m_121096_(localCache.getUnchecked(node)).m_121104_();
                    }
                });
                for(Face f : faces)
                {
                    ...
                    quadBuilder.setQuadOrientation(Direction.m_122372_(f.getNormal().m_122239_(), f.getNormal().m_122260_(), f.getNormal().m_122269_()));
        ...
        private final void putVertexData(IVertexConsumer consumer, Vertex v, Vector3f faceNormal, TextureAtlasSprite sprite)
        {
            ...
            ImmutableList<VertexFormatElement> vertexFormatElements = consumer.getVertexFormat().m_86023_();
            for(int e = 0; e < vertexFormatElements.size(); e++)
            {
                switch(vertexFormatElements.get(e).m_86048_())
                {
                case POSITION:
                    consumer.put(e, v.getPos().m_122239_(), v.getPos().m_122260_(), v.getPos().m_122269_(), 1);
                    break;
                case COLOR:
                    if(v.getColor() != null)
                    {
                ...
                case UV:
                    ...
                    if(vertexFormatElements.get(e).m_86049_() < v.getTexCoords().length)
                    {
                        consumer.put(e,
                            sprite.m_118367_(v.getTexCoords()[0].m_123601_() * 16),
                            sprite.m_118393_(v.getTexCoords()[0].m_123615_() * 16),
                            0,
                            1
                        );
                    }
                ...
                case NORMAL:
                    if(v.getNormal() != null)
                    {
                        consumer.put(e, v.getNormal().m_122239_(), v.getNormal().m_122260_(), v.getNormal().m_122269_(), 0);
                    }
                    else
                    {
                        consumer.put(e, faceNormal.m_122239_(), faceNormal.m_122260_(), faceNormal.m_122269_(), 0);
                    }
                    break;
                default:
                    consumer.put(e);
